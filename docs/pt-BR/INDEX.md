...
[07 — Endurecimento biométrico](07-biometric-hardening.md); trate como a referência de
profundidade e tom.

Termos técnicos (token, hash, endpoint, challenge, nonce, bypass, deny-list, payload, JWT,
JWKS, JWS, mutex, race condition, replay, step-up, etc.) ficam em inglês — é o vocabulário
real que devs brasileiros usam no dia a dia.
...
| 02 | [Cofre de tokens com lastro de hardware](02-hardware-vault.md) — Keystore + StrongBox | ⏳ |
...
| 07 | [Endurecimento biométrico + intenção do usuário](07-biometric-hardening.md) — binding via `CryptoObject` | ✅ |
...
- Termos técnicos de mercado ficam em inglês: token, hash, endpoint, nonce, challenge, bypass,
  ...
  stack trace, log, thread, deadlock, refresh, attestation, signing, verifier.
- "Fortress field notes" não traduz (é o nome da marca de campo do projeto).
